...
function get_valid_moves(board, i, query, game){
    ...
    let valid_moves = MOVE_DATA[piece](board, i, color, game);
...
function move_piece_on_board(board, from, to, query, game){
    let from_1d = i_1D(from[0], from[1]);
    let to_1d = i_1D(to[0], to[1]);

    if(query){
        set_move_indicators(board, from_1d, to_1d, true, game);
        return board.join('/');
    } 
    ...
    set_move_indicators(board, from_1d, to_1d, false, game);
...
function select_pawn_upgrade(board, tile, type, game){
